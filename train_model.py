...
def print_program(inp):
    inp = recover_measure_input(inp, True)
    print("=" * 60)
    print(inp.state)


def evaluate_model(model, test_set, prog_inputs, prog_results):
    # make 
    ...
    for i in range(10):
        print('------------------------')
        print(f"Program No.{i}, With Schedule:")
        print_program(prog_inputs[i])
        print(f"Predicted Throughput: {preds[i]}, Speedup Rate: {preds[i] / base_throughput}")
    print("=========== Valid ============")
...
def make_model(name, use_gpu=True, max_depth=None, gamma=None, min_child_weight=None, eta=None):
    """Make model according to a name"""
    if name == "xgb":
        model = XGBModelInternal(use_gpu=use_gpu)
        
        model.xgb_params = {
            # 树的最大深度，控制树的复杂度，防止过拟合（取值范围：正整数1~10）
            "max_depth": max_depth if max_depth != None else 2,  
            # 节点分裂的最小损失减少值，值越大越能避免过拟合（取值范围：0~0.1）
            "gamma": gamma if gamma != None else 0.003,  
            # 子节点所需的最小样本权重和，用于防止过拟合（取值范围：0~4）
            "min_child_weight": min_child_weight if min_child_weight != None else 2,  
            # 学习率，控制每次更新的步长，较小的eta需要更多轮的迭代（取值范围：0~0.2）
            "eta": eta if eta != None else 0.2,  
            
            "n_gpus": 1,  # 使用的GPU数量（不要改动）
            "nthread": 8,  # 用于运行的线程数（不要改动）
            "verbosity": 0,  # 日志信息的输出级别（不需要改动）
            "seed": 86,  # 随机数种子，用于确保结果的可重复性
            "disable_default_eval_metric": 1,  # 是否禁用默认评估（不需要改动）
        }
        
        if use_gpu:
            model.xgb_params['tree_method'] = 'gpu_hist'
        return model
...
def train_zero_shot(dataset, train_ratio, model_names, split_scheme, use_gpu, number, max_depth, min_child_weight, gamma, eta, prog_inputs, prog_results):
    # Split dataset
    ...
    models = []
    for name in names:
        models.append(make_model(name, use_gpu, max_depth, min_child_weight, gamma, eta))

    eval_results = []
    for name, model in zip(names, models):
        # Train the model
        filename = f"/home/user{number}/tenset/scripts/" + name + ".pkl"
        model.fit_base(train_set, valid_set=test_set)
        # print("Save model to %s" % filename)
        # model.save(filename)
        print("======TRAIN MODEL OVER=======")
        # Evaluate the model
        eval_res = evaluate_model(model, test_set, prog_inputs, prog_results)
...
if __name__ == "__main__":
    ...
    train_zero_shot(dataset, args.train_ratio, args.models, args.split_scheme, args.use_gpu, args.number, args.max_depth, args.min_child_weight, args.gamma, args.eta)
